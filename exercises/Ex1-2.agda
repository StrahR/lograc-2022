...
5-is-not-even (even-ss (even-ss ()))

4-is-even : Even 4
4-is-even = even-ss (even-ss even-z)
...
from-ne (⟨⟩ O) neO = zero
from-ne (b O O) neO = double (from-ne (b O) neO)
from-ne (b I O) neO = double (from-ne (b I) neI)
from-ne (⟨⟩ I) neI = 1
from-ne (b O I) neI = suc (double (from-ne (b O) neO))
from-ne (b I I) neI = suc (double (from-ne (b I) neI))
...
data _≤ᴸ_ {A : Set} : List A → List A → Set where
  ...
  e≤ᴸl : {xs : List A} → [] ≤ᴸ xs
  l≤ᴸl : {x y : A} {xs ys : List A} → xs ≤ᴸ ys → x ∷ xs ≤ᴸ y ∷ ys
...
double-is-even {zero} = even-z
double-is-even {suc n} = even-ss double-is-even


from-even : {b : Bin} → Even₂ b → Even (from b)
from-even {b O} even₂ = double-is-even

≡ᴺ-comm : {m n : ℕ} → m ≡ᴺ n → n ≡ᴺ m
≡ᴺ-comm z≡ᴺz = z≡ᴺz
...
+-zero : {m : ℕ} → m ≡ᴺ m + 0
+-zero {zero} = z≡ᴺz
+-zero {suc m} = s≡ᴺs +-zero

+-suc : {m n : ℕ} → m + suc n ≡ᴺ suc (m + n)
+-suc {zero} {n} = ≡ᴺ-refl
+-suc {suc m} {n} = s≡ᴺs +-suc

+-comm : {m n : ℕ} → m + n ≡ᴺ n + m
+-comm {zero} {n} = +-zero
+-comm {suc m} {zero} = s≡ᴺs (≡ᴺ-comm +-zero)
+-comm {suc m} {suc n} = s≡ᴺs (≡ᴺ-trans (≡ᴺ-trans (+-suc {m} {n}) (s≡ᴺs (+-comm {m} {n}))) (≡ᴺ-comm (+-suc {n} {m})))